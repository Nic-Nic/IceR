...
#' @export
add_isotope_features <- function(path_to_features,feature_table_file_name=def_feat_tab_fname,min_observations=0)
{
...
Alignment_scoring <- function(peaks, features_select, samples, sd_RT=0.5,
    ...
    # and feature
    if(corrected_alignment == TRUE) loc <- c("feature", "sample", "RT_correct")
    if(corrected_alignment == FALSE) loc <- c("feature", "sample", "RT")
    ...
    if(corrected_alignment == TRUE) loc <- c("feature", "sample", "mz_correct")
    if(corrected_alignment == FALSE) loc <- c("feature", "sample", "mz")
...
if(any(c1 & c2)){
    ...
    Mono_iso_alignment_scoring <- function(features, samples, peaks,
        ...
        if(length(which(peaks$known == 1)) >= 10){
            ...
            # peak selection based on detected deviations to isotope peak selection
            if(plot == TRUE){
